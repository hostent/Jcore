...
public abstract class DbSetOracle<T> implements ISet<T>,IDbQuery<T> {
	...
	public List<T> QueryXml(String reportName, Hashtable<String, Object> par) {
		...
		List<T> list = new ComplexSqlHelp<T>(conn, getType(),"oracle").GetReportData(reportName, par);
	...
	public PageData<T> QueryXml(String reportName, PagePars param) {
		...
		PageData<T> result = new ComplexSqlHelp<T>(conn, getType(),"oracle").GetReportData(reportName, param.PageSize, param.PageIndex,
