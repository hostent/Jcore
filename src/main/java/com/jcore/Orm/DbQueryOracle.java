...
public class DbQueryOracle<T> implements IQuery<T> ,IDbQuery<T>{

	public DbQueryOracle(String connKey, Class<?> type) {
		ConnKey = connKey;
		TEntity = new Entity<T>(type);
		trackSql = trackSql.replace("{table}", "\""+TEntity.tableName+"\"");
	}
	
	public DbQueryOracle(String connKey, Class<?> type, String prefix) {
		ConnKey = connKey;
		TEntity = new Entity<T>(type);
